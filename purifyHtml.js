...
export default function purifyHTML(input, allowed) {
    ...
    d.innerHTML = stripTags(
        `<span>${input}</span>`,
        allowed && !allowed.includes('<span>') ? `<span>${allowed}` : allowed
    );
    const sel = d.childNodes[0].querySelectorAll('*');
    for (let i = 0; i < sel.length; i++) {
        ...
        const removeAttrs = [];
        for (let j = 0; j < sel[i].attributes.length; j++) {
            const attrib = sel[i].attributes[j];
            if (attrib.specified) {
